...
- **Nunca simular pagos.** Nada de "cargo procesado" falso ni saldo en localStorage. Métodos válidos en BD: `cafeteria` (solo vía `pay_with_cocinarte_card`: saldo real, débito y comanda en una transacción) y `caja` (efectivo al recoger, solo si todos los items tienen `menu_items.pago_en_caja_permitido`; lo valida `create_comanda`). `pasarela` es un prototipo del checkout (`src/lib/payments/mockGateway.ts`): no cobra, no escribe en Supabase y nunca crea pedidos; `create_comanda` la rechaza. Oculta en producción salvo `VITE_PUBLIC_ENABLE_MOCK_GATEWAY=true`. Solo staff recarga saldo (`staff_recharge_card`).
