...
- **Nunca simular pagos.** Nada de "cargo procesado" falso ni saldo en localStorage. Métodos válidos en BD: `cafeteria` (solo vía `pay_with_cocinarte_card`: saldo real, débito y comanda en una transacción) y `caja` (efectivo al recoger, solo si todos los items tienen `menu_items.pago_en_caja_permitido`; lo valida `create_comanda`). `pasarela` es la opción "Tarjeta · Débito o crédito" del checkout en modo demostración (`src/lib/payments/mockGateway.ts`): no cobra, no escribe en Supabase y nunca crea pedidos; `create_comanda` la rechaza. Visible siempre, también en producción. Solo staff recarga saldo (`staff_recharge_card`).
